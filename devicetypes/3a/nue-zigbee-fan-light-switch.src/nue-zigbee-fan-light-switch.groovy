/**
Modifed 20201217 By stikb78 (forked)
...
Date: 12/07/2019
Updated by Kevin X to fix the issue that switch status failed to update on the APP on 12 Jun 2020
*/
...
log.debug "onoff"
def refreshCmds = zigbee.readAttribute(0x0006, 0x0000, [destEndpoint: 01]) +
zigbee.readAttribute(0x0006, 0x0000, [destEndpoint: 02]) +
zigbee.readAttribute(0x0006, 0x0000, [destEndpoint: 03]) +
zigbee.readAttribute(0x0006, 0x0000, [destEndpoint: 04])
...
[
    "st rattr 0x${device.deviceNetworkId} 0x01 0x0006 0x0", "delay 2",
    "st rattr 0x${device.deviceNetworkId} 0x02 0x0006 0x0", "delay 2",
    "st rattr 0x${device.deviceNetworkId} 0x03 0x0006 0x0", "delay 2",
    "st rattr 0x${device.deviceNetworkId} 0x04 0x0006 0x0", "delay 2",
]
}
/*
def refresh() {
...
def configure() {
	...
	//sendEvent(name: "checkInterval", value: 2 * 1 * 10 + 1 * 10, displayed: false, data: [protocol: "zigbee", hubHardwareId: device.hub.hardwareID])

	// OnOff minReportTime 0 seconds, maxReportTime 5 min. Reporting interval if no activity
	refresh() + zigbee.onOffConfig(0, 3)
}